...
;(function () {
  ...
  // ─── Keep Netflix UI active while our panel is open ───
  // Netflix hides the cursor and controls after ~3s of no mouse movement.
  // We dispatch a synthetic mousemove to prevent that.
  ...
  function inject() {
    ...
    root.innerHTML = `
      ...
      <div id="fl-panel" class="fl-hidden">
        ...
        <div id="fl-auth-gate" style="display:none">
          <p class="fl-empty-msg">Sign in via the FlixList extension popup (top-right of browser) to use playlists.</p>
        </div>
    ...
    // ─── Block ALL keyboard events from reaching Netflix while panel is open ───
    ...
    function closeP() {
      panelOpen = false; panel.classList.add('fl-hidden'); toggle.classList.remove('fl-active')
      stopKeepAlive()
    }
    ...
    async function showList() {
      ...
      if (playlists?.error) {
        const msg = (playlists.error.includes('expired') || playlists.error.includes('sign in'))
          ? 'Session expired — open extension popup to sign in again.'
          : playlists.error
        listEl.innerHTML = `<div class="fl-empty-msg">${esc(msg)}</div>`
      ...
      // ─── Save section — only on watch pages ───
      if (isWatchPage() && playlists && !playlists.error) {
        ...
        if (playlists.length) {
          ...
          saveList.innerHTML = '<div class="fl-empty-msg" style="padding:4px 0;font-size:11px;">Create a playlist first.</div>'
        }
      } else {
        saveSection.style.display = 'none'
      }
    ...
    async function doSave(pid, pname, info) {
      ...
      try {
        ...
        if (r?.error) {
          st.textContent = 'Error: ' + r.error; st.className = 'fl-save-status fl-error'
        } else {
          st.textContent = `✓ Saved to "${pname}"`; st.className = 'fl-save-status fl-success'
        }
      } catch (err) {
        st.textContent = 'Error saving'; st.className = 'fl-save-status fl-error'
        console.error('FlixList save error:', err)
      }
