...
async function saveTokens(a, r) { await chrome.storage.local.set({ flixlist_token: a, flixlist_refresh: r }) }
async function clearAuth() { await chrome.storage.local.remove(['flixlist_token', 'flixlist_refresh', 'flixlist_user']) }
...
async function refreshSession() {
  ...
  await clearAuth(); return null
}

async function getValidToken() {
  const { access } = await getTokens()
  if (!access) return null
  try { const p = JSON.parse(atob(access.split('.')[1])); if (Date.now() < p.exp * 1000 - 30000) return access } catch {}
...
async function supaFetch(path, options = {}) {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, {
    ...options,
    headers: { 'Content-Type': 'application/json', apikey: SUPABASE_ANON, Authorization: `Bearer ${token || SUPABASE_ANON}`, Prefer: options.prefer || 'return=representation', ...options.headers },
  })
  if (!res.ok) {
    const err = await res.text()
    if (res.status === 401 || res.status === 403) { await clearAuth(); throw new Error('Session expired — please sign in again') }
...
async function signUp(email, password, username) {
  try {
    ...
    if (data.id || data.access_token) {
      if (data.access_token) { await saveTokens(data.access_token, data.refresh_token); await chrome.storage.local.set({ flixlist_user: data.user }) }
      return { success: true }
    }
...
async function getPlaylistItems(pid) { return supaFetch(`playlist_items?playlist_id=eq.${pid}&order=created_at.asc`) }
...
async function addToPlaylist(pid, item) { return supaFetch('playlist_items', { method: 'POST', body: JSON.stringify({ playlist_id: pid, ...item }) }) }
async function removeFromPlaylist(iid) { return supaFetch(`playlist_items?id=eq.${iid}`, { method: 'DELETE' }) }
async function updateItem(iid, updates) { return supaFetch(`playlist_items?id=eq.${iid}`, { method: 'PATCH', body: JSON.stringify(updates) }) }
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  const h = {
    ...
    GET_USER: async () => { const t = await getValidToken(); if (!t) { await clearAuth(); return null }; const { flixlist_user } = await chrome.storage.local.get('flixlist_user'); return flixlist_user || null },
  }
