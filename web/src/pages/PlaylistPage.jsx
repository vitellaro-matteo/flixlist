...
import {
  getPlaylist, addItem, removeItem, updateItem, reorderItems,
  toggleLike, hasLiked, updatePlaylist, deletePlaylist,
...
export default function PlaylistPage({ user }) {
  ...
  // STABLE ordered items — only set on load and explicit reorder/add/remove
  ...
  async function load() {
    ...
    if (data) {
      ...
      // Sort items ONCE on load
      const sorted = [...(data.playlist_items || [])].sort((a, b) => {
  ...
  async function handleRenameItem(itemId) {
    ...
    if (!newTitle) return
    // Update locally WITHOUT triggering a full re-sort
    setItems(prev => prev.map(i => i.id === itemId ? { ...i, title: newTitle } : i))
    await updateItem(itemId, { title: newTitle })
  }

  async function moveItem(fromIndex, direction) {
    ...
    setItems(newOrder)
    await reorderItems(id, newOrder.map(i => i.id))
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-8 py-10">
      ...
      <div className="space-y-2 mb-6 fade-up stagger-2">
        {items.map((item, i) => (
          <div key={item.id} className="card !rounded-lg flex items-center gap-3 p-4 group hover:!border-[var(--border-light)]">
            {/* Reorder buttons (owner only) */}
            {isOwner && (
              <div className="flex flex-col gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => moveItem(i, -1)} disabled={i === 0}
                  className="p-0.5 rounded hover:bg-white/5 disabled:opacity-20" style={{ color: 'var(--t3)' }}><ChevronUp size={12} /></button>
                <button onClick={() => moveItem(i, 1)} disabled={i === items.length - 1}
                  className="p-0.5 rounded hover:bg-white/5 disabled:opacity-20" style={{ color: 'var(--t3)' }}><ChevronDown size={12} /></button>
              </div>
            )}
            <span className="w-6 text-center text-sm font-mono shrink-0" style={{ color: 'var(--t3)' }}>{i + 1}</span>
            <div className="flex-1 min-w-0">
              {renamingItemId === item.id ? (
                <div className="flex items-center gap-2">
                  <input value={renameValue} onChange={e => setRenameValue(e.target.value)}
                    className="input-field !py-1 !text-sm flex-1" autoFocus
                    onKeyDown={e => { if (e.key === 'Enter') handleRenameItem(item.id); if (e.key === 'Escape') setRenamingItemId(null) }} />
            ...
            <div className="flex items-center gap-1.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              {isOwner && renamingItemId !== item.id && (
                <button onClick={() => { setRenamingItemId(item.id); setRenameValue(item.title) }}
                  className="p-1.5 rounded-md hover:bg-white/5" style={{ color: 'var(--t3)' }} title="Rename"><Pencil size={13} /></button>
              )}
              {item.netflix_url && (
                <a href={item.netflix_url} target="_blank" rel="noopener"
                  className="p-1.5 rounded-md hover:bg-white/5" style={{ color: 'var(--accent)' }} title="Open in Netflix"><ExternalLink size={14} /></a>
              )}
              {isOwner && (
                <button onClick={() => handleRemoveItem(item.id)} className="p-1.5 rounded-md hover:bg-white/5" style={{ color: 'var(--t3)' }} title="Remove"><Trash2 size={14} /></button>
              )}
            </div>
          </div>
        ))}
      ...
      {items.length === 0 && (
        <div className="text-center py-12 fade-in" style={{ border: '1px dashed var(--border)', borderRadius: 12 }}>
          ...
          <p className="text-xs" style={{ color: 'var(--t3)' }}>
            {isOwner ? 'Add items below, or save from Netflix using the extension.' : "The author hasn't added items yet."}
          </p>
        </div>
      )}
      ...
      <div className="mt-10 fade-up stagger-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0"
                    style={{ background: 'var(--accent)', color: 'white' }}>{(c.profile?.username || '?')[0].toUpperCase()}</div>
                  <span className="text-xs font-medium" style={{ color: 'var(--t2)' }}>{c.profile?.username || 'anon'}</span>
                  <span className="text-xs" style={{ color: 'var(--t3)' }}>{c.created_at ? formatDistanceToNow(new Date(c.created_at), { addSuffix: true }) : ''}</span>
                </div>
                {user && c.user_id === user.id && (
                  <button onClick={() => handleDeleteComment(c.id)} className="p-1 rounded hover:bg-white/5 shrink-0" style={{ color: 'var(--t3)' }}><Trash2 size={12} /></button>
                )}
              </div>
        ...
        {user ? (
          <form onSubmit={handlePostComment} className="flex gap-2">
            ...
            <button type="submit" disabled={postingComment || !commentText.trim()} className="btn-primary !px-4 shrink-0"><Send size={14} /></button>
          </form>
        ) : (
          <p className="text-sm text-center py-3" style={{ color: 'var(--t3)' }}><Link to="/auth" style={{ color: 'var(--accent)' }}>Sign in</Link> to leave a comment.</p>
        )}
      </div>
    </div>
  )
